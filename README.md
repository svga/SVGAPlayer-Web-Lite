...
“动画检验台”。页面动态读取 `tests/fixtures/svga/` 中的 17 个真实业务文件，
可以完整测试单个动画、复用已挂载实例进行热播放，或顺序巡检全部素材。其中
`show.svga` 是保留的 SVGA 1.x 素材，页面应把它显示为预期版本拒绝；其余 16
个文件应完成解析和渲染。

页面分别报告素材复杂度、文件读取、Worker 解析、图片解码与挂载、首帧、实际
帧推进 FPS、跳帧、帧间隔分位数、长任务和可用的内存数据。解析耗时包含 Blob
读取、解压、Protobuf 解码和 Worker 回传；帧间隔反映端到端播放节奏，不代表
渲染函数本身的纯执行时间。长任务和内存接口不受所有浏览器支持，不可用时页面
会明确标注，而不会用 `0` 代替。性能数值用于同一设备和浏览器中的诊断与版本
对比，不是持续集成的固定耗时门槛。
