...
命令会先构建真实 UMD 产物，再在 `http://127.0.0.1:4173` 启动仅供本地维护的
“动画检验台”。默认基线为 npm 的 `latest`，启动时会解析并显示其**确切版本**；
首次获取后会连同完整性信息缓存。需要固定比较对象时，传入精确版本：

```sh
npm run visual -- --baseline 2.1.1
```

缓存有效时可离线继续使用。若缓存缺失、离线或 registry 不可用，页面会明确把基线
标为不可用；本地单版本诊断仍可运行，双版本比较、全量比较和 JSON 导出会保持禁用，
不会把旧缓存或本地版本伪装成 npm 基线。自动化浏览器测试的服务固定使用
`--baseline local`，因此 CI 不会为此访问 npm。

页面动态读取 `tests/fixtures/svga/` 中的 17 个真实业务文件。可以运行单个本地
素材；在已挂载实例上再次播放以观察热播放；或对当前素材进行快速 1 轮、稳定 3 轮
（可附带热播放）的双版本比较。全量比较每个可播放版本均使用新的隔离运行器，强制
冷启动并最多采样 2 秒；随时可取消，已完整完成的行会保留并可导出 JSON。`show.svga`
是保留的 SVGA 1.x 素材，应显示为预期版本拒绝；其余 16 个文件应完成解析和渲染。

两版本在每轮共享同一份读取的素材输入，避免把网络和文件读取差异算入运行时差异。
快速 1 轮会明确标为方向性数据；稳定 3 轮以中位数、MAD 离散度、实际差值和结果带宽
显示“改善 / 退化 / 波动带内”，高离散度和样本不足会单独告警。只有元数据、能力、画面指纹和
预期结果均通过正确性门控时，才比较性能。页面分别报告素材复杂度、Worker 解析、
图片解码与挂载、首帧、实际帧推进 FPS、跳帧、帧间隔分位数、长任务和可用的内存数据。
解析耗时包含 Blob 读取、解压、Protobuf 解码和 Worker 回传；帧间隔反映端到端播放
节奏，不代表渲染函数本身的纯执行时间。内存仅适合同机同浏览器的前后趋势；长任务和
内存接口并非所有浏览器都支持，缺失时明确显示不可用而不是用 `0` 代替。

`2.1.1` 基线仅在其隔离运行页临时放宽 `unsafe-eval`，以支持该版本的默认 Worker；
检验台不会启用直接解析模式。本地运行页及其他基线页保持不含 `unsafe-eval` 的 CSP。
所有结果只用于同一机器、同一浏览器下的趋势诊断和版本比较，不是持续集成的耗时门槛。

浏览器自动化会在 Chromium 完成全量巡检并断言 16 个匹配、1 个预期拒绝、0 个失败、
0 个需复核；Chromium、Firefox、WebKit 都会运行代表性的真实素材和操作流程。
